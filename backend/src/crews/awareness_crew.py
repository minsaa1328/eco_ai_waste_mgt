...
if __name__ == "__main__":
    ...
    print("=== Testing Awareness Tips ===")
    tips_contexts = [
        "user recycled a plastic bottle",
        "user composted food waste",
        "user used a reusable bag"
    ]

    for context in tips_contexts:
        tip = crew.get_awareness_tip(context)
        print(f"\nContext: {context}")
        print(f"Tip: {tip}")

    print("\n=== Testing Quiz Questions ===")
    quiz_topics = ["composting", "plastic", "recycling", "general"]
    ...
    for topic in quiz_topics:
        try:
            ...
            print(f"\n❌ Error processing quiz for {topic}: {e}")
            print(f"Raw JSON: {quiz_json}")
            # Use fallback directly
            fallback_json = crew._get_enhanced_fallback_quiz(topic)
            fallback_data = json.loads(fallback_json)
            print(f"Using fallback for {topic}:")
            print(f"Question: {fallback_data['question']}")
            print(f"Options: {fallback_data['options']}")
            print(f"Correct Answer: {fallback_data['correct_answer']}")